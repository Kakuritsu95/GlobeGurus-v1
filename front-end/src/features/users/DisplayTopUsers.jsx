...
function DisplayTopUsers() {
  ...
  return (
    <div className=" xl:w-1/4">
      <div className="mx-auto mt-20 w-2/3 rounded-lg border-2 bg-gray-700">
        <h3 className="mt-7 text-center font-bold text-gray-100">
          OUR TOP GURUS
        </h3>
        <ul className="px-2 py-5 text-gray-100 ">
          <div className="mx-2 my-2 flex rounded bg-gray-600 py-3">
            {users.slice(0, 2).map((user, i) => {
              return (
                <li className="mx-auto flex flex-col text-center">
          ...
          <div>
            {users.slice(2).map((user, i) => {
              return (
                <li className="mx-2 my-3 flex items-center justify-between rounded bg-gray-600 px-14 py-2">
                  <Avatar avatarUrl={user.owner.avatarUrl} />
                  <Link
                    to={`/guides/user/${user.owner._id}`}
